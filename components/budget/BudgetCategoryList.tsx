...
import "../@/resources/budget/budgetCategoryList.scss";
...
export default class BudgetCategoryList extends React.Component<any, any> {
  ...
  getBudgetedAmountCell(rowId, amount) {
    ...
    if (this.isRowInEdition(rowId)) {
      cellContent = (
        <div className="editBudgetOnTable">
          <TextField
            size="small"
  ...
  handleUpdateRetry = () => {
    this.setState({ operationModalVisible: false, operationData: null });
  };

  calculateProportion(spent, budget) {
    return budget == -1 || budget == 0
      ? "-"
  ...
  finishBudgetEditionOnTable = (event, forced) => {
    ...
    let data = {
      id: this.state.rowCurrentlyInEdition,
      budget: this.state.budgetOnEditionValue,
    };

    this.props
      .updateCategoryBudget(data.id, data.budget, forced)
      .then((result) => {
        console.log("hola");
        console.log(result);
          ...
          <Table size="small" aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="left">
                  <span className="columnName">Gasto actual</span>
                </TableCell>
                <TableCell align="left">
                  <span className="columnName">Presupuesto</span>
                </TableCell>
                <TableCell align="left">
                  <span className="columnName">Porcentaje gastado</span>
                </TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
                  ...
                  <React.Fragment key={row.category.categoryId}>
                    ...
                    >
                      <TableCell style={{ borderBottom: 0, maxWidth: "50px" }}>
                        ...
                        >
                          {this.isRowExpanded(row.category.categoryId) ? (
                            <KeyboardArrowUpIcon />
                          ) : (
                    ...
                    <TableRow>
                      ...
                      >
                        <Collapse
                          in={this.isRowExpanded(row.category.categoryId)}
                          timeout="auto"
                          unmountOnExit
                        ></Collapse>
                      </TableCell>
        ...
        >
          <Fade in={this.state.operationModalVisible}>
            <div className="confirmUpdateModalContent">
              ...
              <p className="modalInnerParagraph">
                Aumentar el presupuesto en el monto indicado para esta categoria
                implica superar el presupuesto total.
              </p>
              <p className="modalInnerParagraph">
                Desea ingresar un monto nuevamente o actualizar el presupuesto
                total?
              </p>
