...
export default class CategoryButton extends React.Component<any, any> {
  ...
  deleteCategoryFromTask = () => {
    if (this.state.task !== null && this.state.task.operation === "del") {
      deleteCategory(this.props.categoryId).then((res) => {
        if (res.result != "error") this.handleCategoryTaskFinished();
      });
    }
  };
  ...
  toggleOperation = () => {
    ...
    if (operation == "add") this.handleAddCategory();
    else if (operation == "edit") this.handleEditCategory();
    else if (operation == "del") this.handleDelCategory();
  };
