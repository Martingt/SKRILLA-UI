...
import {getbudgetPerCategory} from '../controllers/budgetsController'
import {XYPlot,
    XAxis,
    YAxis,
    VerticalGridLines,
...
export default class BudgetPieChart extends React.Component<any, any>  {
  ...
  fetchData(){
    fetchBudgetSummary()
    .then((result)=> {this.setState({...this.state,budgetItems: result.categoryItems})})
  ...
  createBudgetData(){
    ...
    if(this.state.budgetItems.length!=0){
      this.state.budgetItems.map((budget) => {
        if(budget.budgetedAmount>0){
            data.push({x: budget.category.name, y: budget.budgetedAmount-budget.totalSpent})
        }
      })
    }
    return data
  ...
  tooltipData(cell){
      for(const i in this.state.budgetItems){
          if(this.state.budgetItems[i].category.name==cell.x){
              const percentage = (this.state.budgetItems[i].totalSpent / this.state.budgetItems[i].budgetedAmount)*100;
    ...
    if(this.state.budgetItems.length > 0){
      ...
      return <div className="content">
        ...
        <div style={{width: 400, height: 300}} className="noChart">
            <img style={{width: 160, height: 150, marginBottom: 10, marginTop: 40}} src="/images/no-budget.png"></img>
            <h2>Ups!</h2>
            <p style={{textAlign:"center",marginTop: 10}}>No tienes presupuesto armado!</p>
        </div>
      </div>
    }
  }
