...
class ConsumptionView extends React.Component<any, any> {
  ...
  handleChange(event){
    ...
    event.target.name == 'month'? this.setState({[event.target.name]: event.target.value}, ()=>this.filterByDate())
      : event.target.name == 'year'? this.setState({[event.target.name]: event.target.value}, ()=>this.filterByDate())
      : ""
  }
  filterByCategory = (e) => {
  ...
  filterByDate = () => {
    var day;
    ([1,3,5,7,8,10,12].includes(this.state.month)) ? day = 31 : ([4,6,9,11].includes(this.state.month)) ? day = 30: day = 28
    fetchConsumptionsPeriod(this.state.year+"-"+this.state.month+"-01",this.state.year+"-"+this.state.month+"-"+day)
      .then((result) => {
        this.setState({ ...this.state, consumptions: result });
      })
      .catch((error) => console.log("error", error));
          ...
          <div className="mainContainerContent">
            ...
            <div className="containerToolbar">
              <div className="topBarFilters">
                ...
                <Select className="monthFilter" inputProps={{name: "month"}} onChange={this.handleChange}>
                  {month.map((month, key) => { return <MenuItem value={key+1}>{month}</MenuItem>})}
                </Select>
                <Select className="yearFilter" inputProps={{name: 'year'}} onChange={this.handleChange}>
                  {year.map((year, key) => { return <MenuItem value={2020-key}>{year}</MenuItem>})}
                </Select>
                <a className="clearFilter" onClick={this.filterByCategory}> Clear Filters</a>
              </div>
